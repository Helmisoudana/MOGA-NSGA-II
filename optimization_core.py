...
Notes:
  - Ensures DEAP requirements (crowding distance computed, population size divisible by 4).
  - Accepts initial_population as list-of-dicts, list-of-lists, or pandas.DataFrame.
  - If DEAP missing, falls back to a simple evolutionary routine.
"""
...
DEFAULT_BOUNDS = [
    ...
    (400.0, 1200.0),  # masse_châssis_kg
    (0.18, 0.40),     # Cd
    (1.8, 3.0),       # surface_frontale_m2
    (0.8, 1.2),       # facteur_roue
]
...
if DEAP_AVAILABLE:
    ...
    try:
        creator.FitnessMin  # type: ignore
    except Exception:
...
def run_nsga2(config: Dict,
    ...
    if DEAP_AVAILABLE:
        ...
        # Genetic operators (bounded SBX and polynomial mutation recommended)
        ...
        for gen in range(1, ngen + 1):
            # --- Important DEAP NSGA-II prerequisites ---
            ...
            # Selection for mating using Tournament DCD (requires crowding_dist)
            ...
            # Build current pareto front (convert DEAP fitness tuples to numeric list)
            pop_dicts = []
            for ind in pop:
        ...
        # Filter nondominated among final_pop_dicts (use raw minimization vector)
        temp_for_filter = []
        ...
        # ----------------------------
        # Fallback simple evolutionary strategy (random search + pareto selection)
        # ----------------------------
        ...
        # build final pareto_solutions (convert objectives sign as before)
